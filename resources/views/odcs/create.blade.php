...
@section('title', 'Tambah ODC - WiFi Billing Management')
...
<div class="space-y-6 lg:space-y-8 max-w-5xl mx-auto">
    <div class="page-header">
        <div class="flex items-center gap-3 sm:gap-4">
            ...
            <div>
                <h1 class="page-header__title text-slate-900">Tambah ODC Baru</h1>
                <p class="mt-1 text-xs sm:text-sm text-gray-500">Definisikan ODC beserta lokasi koordinat dan kapasitasnya</p>
            </div>
        </div>
    ...
    <form method="POST" action="{{ route('odcs.store') }}" class="space-y-6">
        ...
        <div class="app-card space-y-5">
            ...
            <div>
                ...
                <textarea name="alamat"
                          ...
                          placeholder="Alamat lengkap ODC">{{ old('alamat') }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="app-card inline-actions">
            <button type="submit"
                    class="inline-flex items-center justify-center px-6 py-3 text-sm font-semibold bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl hover:shadow-lg transition">
                <i class="fas fa-save mr-2"></i>Simpan ODC
            </button>
            <a href="{{ route('odcs.index') }}"
