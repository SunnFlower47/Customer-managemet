            ...
            <div class="app-card space-y-4">
                ...
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700">
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
                        <dt class="text-xs font-semibold text-blue-700 uppercase tracking-wide mb-1">Port Terpakai</dt>
                        <dd class="text-lg font-bold text-blue-900">{{ $odp->port_terpakai }}/{{ $odp->kapasitas }} Port</dd>
                        @php
                            $activePelanggans = $odp->pelanggans->where('status', 'aktif')->count();
                            $childOdpsCount = $odp->childOdps->count();
                        @endphp
                        <p class="text-xs text-blue-600 mt-1">
                            ({{ $activePelanggans }} pelanggan + {{ $childOdpsCount }} ODP child)
                        </p>
                    </div>
                    ...
                    <div class="bg-purple-50 border border-purple-100 rounded-xl px-4 py-3">
                        <dt class="text-xs font-semibold text-purple-700 uppercase tracking-wide mb-1">Terhubung ke ODP</dt>
                        <dd class="text-sm text-gray-900">
                            <div class="flex items-center justify-between gap-2">
                                <div>
                                    <p class="font-semibold">{{ $odp->parentOdp->kode_odp }}</p>
                                    <p class="text-xs text-gray-600">{{ $odp->parentOdp->nama }}</p>
                                </div>
                                <a href="{{ route('odps.show', $odp->parentOdp) }}" class="text-purple-600 hover:underline text-xs font-semibold">
        ...
        <div>
            ...
            <div class="app-card mt-6">
                <div class="flex items-center justify-between mb-3">
                    <div>
                        <p class="text-xs uppercase tracking-wide text-purple-500 font-semibold">ODP Child Terhubung</p>
                        <h2 class="text-base font-semibold text-gray-900">Daftar ODP yang terhubung ke ODP ini</h2>
                    </div>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold bg-purple-50 text-purple-700 border border-purple-100">
                        {{ $odp->childOdps->count() }} ODP
                    </span>
                </div>
                <div class="space-y-2">
                    @foreach($odp->childOdps as $childOdp)
                    <div class="flex items-center justify-between p-3 bg-gray-50 border border-gray-100 rounded-xl hover:bg-gray-100 transition">
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-gray-900">{{ $childOdp->kode_odp }}</p>
                            <p class="text-xs text-gray-600">{{ $childOdp->nama }}</p>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold mt-1 {{ $childOdp->status === 'aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ ucfirst($childOdp->status) }}
                            </span>
                        </div>
                        <a href="{{ route('odps.show', $childOdp) }}" class="ml-3 inline-flex items-center justify-center px-3 py-1.5 text-xs font-semibold bg-purple-50 text-purple-700 rounded-lg hover:bg-purple-100 transition">
